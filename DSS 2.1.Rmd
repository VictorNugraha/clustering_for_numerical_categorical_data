...
Jika pada <i>Supervised Machine Learning</i>, data yang digunakan memiliki label dan nantinya sistem mengetahui jawaban atau ouput mana yang benar. Bagaimana jika suatu saat kita dihadapi dengan sebuah data yang tidak memiliki label? Dan tugas kita adalah mencari tahu label yang tepat untuk setiap data yang kita miliki? Jika kita dihadapi terhadap permasalah itu, kita tidak usah khawatir dikarenakan ada <i>Unsupervised Machine Learning</i>. <i>Unsupervised Machine learning</i> adalah sebuah mesin yang melakukan pengolahan data tanpa memerlukan label ataupun jawaban, nantinya <i>unsupervised machine learning</i> bisa melakukan eksplorasi data dan menemukan struktur data didalamnya. 

Setiap temuan dari <i>unsupervised Machine learning</i> biasanya akan dibagi lagi kedalam beberapa kelompok berdasarkan kemiripan struktur data, tahapan ini biasanya disebut dengan tahapan <i>clustering</i>. <i>Clustering</i> atau pengelompokan yang baik biasanya data-data yang berada didalamnya memiliki kemiripan yang bisa dibilang hampir identik. Hasil dari pengelompokan itu nantinya yang akan menjadi hasil dari metode <i>machine learning</i> ini

## Stigma

Stigma yang menempel pada <i>Unsupervised Machine learning</i> adalah <i>unsupervised machine learning</i> hanya bisa digunakan untuk data-data numerik saja. Hal tersebut mungkin bisa terjadi dikarenakan, banyak sekali implementasi atau contoh dari <i>unsupervised machine learning</i> yang hanya menggunakan data numerik saja, sehingga munculah sebuah stigma tersebut.

Sesungguhnya <i>unsupervised Machine learning</i> juga bisa digunakan terhadap data-data kategorikal, dan pada kesempatan kali ini kita akan mencoba menggunakan <i>unsupervised Machine learning</i> untuk mengelompokan data kategorikal dan data numerik. Tipe algoritma yang akan kita gunakan nantinya bernama <b><i>Partitioning Around Medoids</b></i>.
...
Langkah tersebut akan terus diulang sampai nantinya <b>S > 0</b>, yang berarti tidak adanya pergerakan lagi dari Medoid yang dipilih sebagai titik tengah. Dari titik Medoids yang paling optimum itu, akan dicari lagi objek yang menyerupai dari karakteristik Medoids tersebut. 
...
Data yang akan kita gunakan adalah data karateristik berlian yang baru saja selesai diolah, dari data tersebut nantinya akan coba kita kelompokan untuk membuat beberpa segmentasi produk dari setiap berlian tersebut.
...
* `color`: Warna dari berlian tersebut (J < I < H < G < F < E < D)
...
* Mayoritas dari hasil berlian yang sudah diolah memiliki potongan dengan hasil menengah keatas karena didominasi dengan hasil potongan dengan kategori <i>very good</i> sampai <i>ideal</i>.
* Kualitas warna yang dihasilkan juga bisa dibilang cukup merata walaupun lebih condong dengan kualitas warna yang bisa dikategorikan menengah kebawah.
* Kerjernihan dari batu berlian yang sudah diolah memiliki kerjenihan dengan kualitas menengah kebawah, karena hasilnya paling banyak didominasi dengan kategory <i>SI2</i> sampai <i>VS1</i>.
...
Dari hasil visualisasi histogram plot, kita bisa menarik kesimpulan bahwa, masing-masing numerik variabel memiliki rentan yang cukup variatif dan jumlah obervasinya juga tidak merata.

* Jumlah kadar karat paling banyak dari data kita adalah 0.7 karat.
* Persentasi kedalaman berlian, paling banyak berada disekitaran 60-65%.
...
Dari hasil eksplorasi data, kita mengetahui bahwa total observasi data kita adalah 10,000 baris dan setiap observasi data pastinya memiliki nilai yang berbeda. Agar nantinya hasil dari setiap kelompok terdiri dari data-data yang indentik, kita harus mengetahui jarak antara satu observasi dengan observasi lainnya berdekatan atau tidak.
...
Seperti yang sudah disampaikan pada bagian pendahuluan, tujuan dari melakukan <i>clustering</i> adalah menempatkan setiap observasi data yang ada ke beberapa kelompok yang sesuai dengan karakteristiknya atau bedasrkan kedekatan jarak antar setiap observasi. Pada langkah pertama kita sudah memiliki informasi jarak antar setiap observasi, kita akan memanfaatkan informasi tersebut utuk menentukan berapakah kelompok yang paling optimal. Hasil dari jumlah kelompok nantinya akan digunakan untuk menentukan berapa banyak segmentasi produk yang bisa dibuat berdasarkan data hasil olahan berlian.
...
Fungsi di atas akan membantu kita untuk mengelompokan keseluruhan observasi data ke dua kelompok bedasarkan kemiripan setiap observasi terhadap titik pusat atau <i>medoids</i>. Supaya kita bisa mendapatkan intuisi awal bagaimana karakteristik olahan berlian untuk setiap kelompoknya, kita bisa melihat dari tabel di bawah ini.
...
Tujuan melakukan statistik deskriptif adalah untuk memahami karakteristik masing-masing kelompok, dan dalam kasus ini untuk mengetahui karateristik untuk setiap segmentasi produk hasil olahan berlian.
...
* Dari sudut pandang kualitas, hasil olahan berlian kelompok 1 didominasi dengan kualitas potongan yang sangat baik akan tetapi untuk hasil warna dan kejernihannya kurang baik.
...
Dari hasil plot di atas, kita bisa melihat bahwa setiap observasi sudah berhasil terbagi menjadi dua kelompok walaupun memang masih ada beberapa obverasi yang tumpang tindih atau berdekatan pada sebuah kelompok yang didominasi oleh kelompok yang berlawanan.

# Kesimpulan

Di sini kita bisa menarik kesimpulan bahwa <i>unsupervised machine learning</i> bisa digunakan untuk melakukan pengelompokan tidak hanyak untuk data numerikal saja melainkan bisa juga digunakan untuk data kategorikal. Walaupun memang ada beberapa pendekatan yang berbeda dalam melakukan pengelompokan terutama metode yang digunakan dalam melakukan perhitungan jarak antara setiap observasi data, akan tetapi secara alur kerja bisa dibilang tidak ada perbedaan sama sekali.

Dalam kasus ini, implementasi dari <i>unsupervised machine learning</i> sangat berguna untuk sebuah perusahaan berlian yang baru berdiri dalam menentukan segmentasi produk karena <i>unsupervised machine learning</i> akan membantu kita dalam memnentukan jumlah segmentasi yang paling optimum dan akan mengelompokan setiap hasil olahan berlian ke setiap kelompok yang memiliki karakteristik yang sama. 
